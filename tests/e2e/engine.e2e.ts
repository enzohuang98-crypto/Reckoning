 ...
 *
 * 執行：npx tsx --tsconfig tsconfig.node.json tests/e2e/engine.e2e.ts
...
async function main(): Promise<void> {
  ...
  section('EngineOutputParser：convertCpScore / convertMateScore（§2.14.6）')
  {
    ...
    check('cp 正分顯示 120', pos.type === 'cp' && pos.displayText === '120' && pos.comparableValue === 1.2)
    ...
    check('cp 負分顯示 -80', neg.displayText === '-80' && neg.comparableValue === -0.8)
  ...
  section('invertEngineScore（§2.15.4）')
  {
    ...
    check('cp 取負後顯示 -42', cp.type === 'cp' && cp.cp === -42 && cp.comparableValue === -0.42 && cp.displayText === '-42')
  ...
  process.env.FAKE_ENGINE_MODE = 'uci'
  {
    ...
    check(
      'scoreAfterBestMove 顯示 42（root_analysis）',
      root.scoreAfterBestMove?.displayText === '42' && root.scoreAfterBestMove.source === 'root_analysis'
    )
    check(
    ...
    check(
      '候選分數未反轉並顯示 15',
      inCand.scoreAfterUserMove?.displayText === '15' && inCand.scoreAfterUserMove.wasInverted === false
    )
  ...
  process.env.FAKE_ENGINE_MODE = 'uci-score-type'
  {
    ...
    check(
      ...
      analysis.scoreAfterBestMove?.type === 'cp' &&
        ...
        analysis.scoreAfterBestMove.displayText === '100',
