...
- 分析永遠是獨立、固定可見的首頁分頁（`AppShell` 的 `.nav-btn`）；其餘目的地（錯題本、待理解、設定）收進同一個「工具」下拉選單（`ToolbarMenu`，與分析工具列的「局面工具」選單共用同一套元件與樣式），避免四個等重分頁攤平在同一列造成的散亂感，窄視窗下也不會整排導覽直接消失。
...
- 錯題本／待理解頁面上方都有搜尋（錯題本另加錯誤等級與理解狀態篩選），沿用 `.list-toolbar` 樣式，篩選結果數與總數並列顯示在頁首右上角。
