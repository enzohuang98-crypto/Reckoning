...
對照文件：`docs/specifications/SDS-v0.2.docx`（軟體設計規格書 v0.2，2026-06-08）
...
| 13 | Q5 買斷授權 License Key | 「是」——需要 License Key | **完全缺失** | ✅ **本次補齊**：離線 Ed25519 簽章驗證（XQA1.payload.sig）、LicenseService（main process）、license:* IPC、啟用頁鎖主介面、設定頁狀態/解除、tools/license/license-keygen.ts 發行工具、18 條測試 |
...
## 第二輪查核（2026-06-11，本 commit）

對 Stage 8（`95c8bf5`）宣稱的對齊做獨立逐項驗證，方法：grep 核對常數與通道名、
逐行核對 §2.17.5 streaming loop 順序規則、§2.16.4 save-先於-reply、§2.19.2 定價表
全 11 模型數字、§2.15.4 mate 0 反轉、§2.15.7 失敗原因字串、§2.17.7 renderer 規則
（requestId 過時事件忽略／partial text 保留／「已取消生成」）。**全部相符**。

發現並補齊的殘留 gap：

| # | SDS 章節 | 規格要求 | 缺漏 | 處置 |
|---|---------|---------|------|------|
| 15 | §2.11、§2.12 | 共用 Logger 模組：自動遮蔽 apiKey、Authorization header、secret token | 無 Logger 模組（雖經查無任何金鑰被 log，但規格要求的防護層不存在） | ✅ 新增 `src/main/logger.ts`（maskSecrets 純函式 + info/warn/error），接上 engine/AI 兩個 IPC handler 的錯誤路徑（取消不記 error）；`tests/unit/main/logger.test.ts` 9 條斷言 |

驗證：typecheck 0 錯、build 綠、五套測試 190 條全過
（rules 49 + providers/registry 37 + license 18 + logger 9 + engine 77），
`npm run dev` 自新路徑（`象棋軟體專案/xiangqi-analyzer`）啟動煙霧測試通過。
