...
| # | SDS 章節 | 規格要求 | 缺漏 | 處置 |
|---|---------|---------|------|------|
| 15 | §2.11、§2.12 | 共用 Logger 模組：自動遮蔽 apiKey、Authorization header、secret token | 無 Logger 模組（雖經查無任何金鑰被 log，但規格要求的防護層不存在） | ✅ 新增 `src/main/Logger.ts`（maskSecrets 純函式 + info/warn/error），接上 engine/AI 兩個 IPC handler 的錯誤路徑（取消不記 error）；`tests/logger.test.ts` 9 條斷言 |
