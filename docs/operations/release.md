...
main 原始碼
  ...
  └─ vX.Y.Z tag
       └─ Release workflow：重跑門檻、封裝、驗證 SHA-256，依 mode 分流
            ...
            ├─ unsigned-release：擁有者明確授權的過渡同步模式，公開下載驗證後升為 Latest
...
6. 若 `mode=teacher-candidate`，workflow 會建立 unsigned prerelease，永不升為 Latest。若 `mode=unsigned-release`，必須輸入 `PUBLISH UNSIGNED LATEST`，並先建立帶警告的 unsigned prerelease；只有同次 build 的 SHA-256、安裝／解除安裝及兩個匿名公開下載代理全部通過，才升為 Latest 供 App 更新。若 `mode=formal-release`，workflow 會先完成有效 Authenticode 簽章與可信時間戳門檻，再公開 signed prerelease。三種模式都記錄檔名、大小、SHA-256、tag、source commit 與 run ID。
