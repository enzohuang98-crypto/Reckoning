...
- **背景準備、明確重啟**：SDK 維持 `autoDownload = false`，由 `AppUpdaterService`
  在主程序以單一佇列控制下載。背景準備預設開啟，可在設定頁關閉；下載期間不打斷對局，
  完成後停在 `downloaded`，只有使用者明確按下重新啟動才會先保存最新資料並安裝。
- **偏好單一來源**：背景準備、跳過與稍後提醒由 main process 原子持久化；升級時
  renderer 只讀一次舊版 localStorage，並在第一次自動檢查前透過 IPC 握手遷移後清除。
- **提示位置**：狀態為 `available`（有新版可下載）或 `downloaded`（已下載待安裝）時，
  `AppShell` 會在標題列顯示提示；新版通知提供「立即背景準備」「4 小時後提醒」
  「跳過此版本」三種選擇。跳過只抑制該版本的自動通知，
