...
`)
    ...
    while (
      ...
      if (failedHeadings.size === 0 && deterministicErrors.length > 0) {
        // 純全篇性錯誤（缺區塊、缺著法連結等）：指向後果與比較區塊重寫
        failedHeadings.set(SECTION_KEYS.consequences, [...deterministicErrors])
      }
...
輸出格式：{"directAnswer":"（僅在被要求時）","sections":[{"heading":"原標題","claims":[{"id":"C2","text":"...","evidenceIds":["E1"],"causal":{"cause":"...","mechanism":"...","affected":"...","opponentUse":"...","consequence":"..."}}]}]}
`)
        )
