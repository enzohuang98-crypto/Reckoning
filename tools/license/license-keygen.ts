 ...
 *
 * 用法：
 *   npx tsx --tsconfig tsconfig.node.json tools/license/license-keygen.ts init
 *     → 產生 Ed25519 金鑰對至 tools/license/keys/（私鑰絕不 commit；目錄已 gitignore），
 *       並印出要內嵌進 src/main/license/LicenseService.ts 的公鑰 PEM。
 *
 *   npx tsx --tsconfig tsconfig.node.json tools/license/license-keygen.ts issue --licensee "王小明"
