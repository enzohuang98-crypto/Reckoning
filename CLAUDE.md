...
src/
  ...
  renderer/                  # React UI（瀏覽器環境，無 Node 權限）
    ...
    src/
      ...
      app/                   # AppShell / StartupScreen / ErrorBoundary
      components/ui/         # 無業務狀態的共用 UI 與 icon
      features/              # analysis / board / explanations / guessing / settings / workspace
      pages/                 # SettingsPage / MistakeBookPage / SetupWizard / LicensePage
      features/board/pieces.ts # 棋子字形與調色盤
      storage/localSettings.ts  # localStorage（設定 + 錯題本）
  shared/                    # main 與 renderer 共用（純型別與純邏輯）
    types/                   # 所有核心型別（見下）
    logic/
      board/                 # FEN、走子規則、中文記譜、timeline
      analysis/              # 走法比較與雙引擎裁決
      ai/                    # 象棋知識、術語與解說品質評分
      validation/            # 跨邊界輸入與 Provider 驗證
